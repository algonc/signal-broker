...
Following instructions explain how to publish signal-broker tutorial APIs via a portable Docker container.
...
- signal-broker project sources locally compiled, and shell pointing to project root.


### Build image
`mvn clean package docker:build`

Now if you issue a `docker images` command you should see `signal-broker:latest` image listed.
...
#### From command line
`docker run -d -p <SERVER_PORT>:8080 signal-broker`
